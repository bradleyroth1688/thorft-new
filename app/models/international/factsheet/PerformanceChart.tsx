...
import { monthlyReturns } from "@/data/international-performance";
...
function buildGrowthData() {
  ...
  const data: { date: string; label: string; thor: number; bench: number }[] = [
    { date: "2016-12-31", label: "Dec 2016", thor: 10000, bench: 10000 },
  ];
...
function formatXAxis(date: string) {
  if (date === "2016-12-31") return "2017";
      ...
      {payload.map((entry) => (
        <p key={entry.dataKey} className="flex items-center gap-2">
          ...
          <span className="text-gray-300">
            {entry.dataKey === "thor" ? "THOR International" : "Vanguard Total World (VT)"}:
          </span>
        ...
        >
          ...
          <Legend
            ...
            formatter={(value: string) =>
              value === "thor" ? "THOR International (Net)" : "Vanguard Total World Stock ETF"
            }
            iconType="line"
          />
          <Area
